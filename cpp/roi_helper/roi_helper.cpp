...
int roi_crop_bgr(uint64_t gst_buffer_ptr,
  ...
  cparams.colorFormat = NVBUF_COLOR_FORMAT_BGR;
  ...
  if (NvBufSurfTransformSetSessionParams(NULL) != 0) {
  ...
  if (NvBufSurfTransform(surf, dst_surf, &tparams, &src_rect, &dst_rect) != 0) {
    NvBufSurfaceDestroy(dst_surf);
    return -8;
  }
  ...
  // Copy row by row in case pitch != width*channels
  for (int y = 0; y < dst_h; ++y) {
    memcpy(out + y * dst_w * 3, ptr + y * pitch, dst_w * 3);
  }
