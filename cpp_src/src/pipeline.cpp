...
namespace EdgeDeepStream
{
    ...
    bool Pipeline::create(const Config &config)
    {
        try
        {
            ...
            if (!gst_bin_add(GST_BIN(pipeline_), sgie_))
            {
                std::cerr << "Failed to add sgie to pipeline" << std::endl;
                return false;
            }
            if (!gst_bin_add(GST_BIN(pipeline_), queue4_))
            {
                std::cerr << "Failed to add queue4 to pipeline" << std::endl;
                return false;
            }
    ...
    bool Pipeline::setup_element_properties(const Config &config)
    {
        ...
        if (config.has_section("pgie"))
        {
            ...
            if (config_file_it != pgie_config.end())
            {
                ...
                g_print("PGIE properties set from config\n");
            }
        }
        ...
        if (config.has_section("sgie"))
        {
            ...
            if (config_file_it != sgie_config.end())
            {
                ...
                g_print("SGIE properties set from config\n");
            }
        }
