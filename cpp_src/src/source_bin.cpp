...
bool SourceBin::create() {
    ...
    // Always use uridecodebin for simplicity and hardware decoder support
...
bool SourceBin::create_uri_decode_source() {
    ...
    // Create uridecodebin with minimal configuration
    ...
    // Set URI - the only required property
    g_object_set(source_element_, "uri", uri_.c_str(), NULL);
    
    // Essential signal connections for hardware decoder
    ...
    // Create ghost pad for dynamic linking
...
void SourceBin::cb_newpad(GstElement* decodebin, GstPad* decoder_src_pad, gpointer data) {
    ...
    // Process only video streams with hardware decoder (NVMM memory)
...
void SourceBin::source_setup(GstElement* uridecodebin, GstElement* source, gpointer user_data) {
    ...
    // Configure RTSP source for optimal 8-stream performance
