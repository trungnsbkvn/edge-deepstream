...
namespace {
    ...
    void save_labels_json(const std::string& filename, const std::vector<std::string>& labels) {
        ...
        try {
            ...
            // If reading fails, start with empty metadata
            metadata = json::object();
        }
        
        // Update the labels array (per-vector mapping)
        metadata["labels"] = labels;
        
        // Remove legacy vector_labels to avoid confusion
        if (metadata.contains("vector_labels")) {
            metadata.erase("vector_labels");
        }
...
std::vector<std::vector<float>> FaceIndex::reconstruct_all() const {
#ifdef HAVE_FAISS
    if (!index_ || labels_.empty()) {
        return {};
    }
    
    std::vector<std::vector<float>> all_vectors;
    all_vectors.reserve(labels_.size());
    
    try {
        // Try to use reconstruct_n if available
        size_t n = labels_.size();
        std::vector<float> flat_data(n * dim_);
        
        // Check if index supports reconstruction
        if (index_->ntotal == static_cast<long>(n)) {
            // Try batch reconstruction first
            try {
                index_->reconstruct_n(0, n, flat_data.data());
                
                // Convert flat data to vector of vectors
                for (size_t i = 0; i < n; ++i) {
                    std::vector<float> vec(dim_);
                    std::memcpy(vec.data(), &flat_data[i * dim_], dim_ * sizeof(float));
        ...
        for (size_t i = 0; i < n; ++i) {
            try {
                std::vector<float> vec(dim_);
                index_->reconstruct(i, vec.data());
                all_vectors.push_back(std::move(vec));
            } catch (...) {
                // Skip failed reconstruction
                continue;
            }
        }
    } catch (const std::exception& e) {
        std::cerr << "Error in reconstruct_all: " << e.what() << std::endl;
    }
...
int FaceIndex::remove_label(const std::string& label) {
#ifdef HAVE_FAISS
    if (labels_.empty()) {
        return 0;
    }
    
    // Count how many instances of this label exist
    int removed_count = 0;
    ...
    // If removing all entries, create empty index
    if (keep_indices.empty()) {
        labels_.clear();
        if (!create_index()) {
            std::cerr << "Failed to recreate empty index" << std::endl;
        }
        return removed_count;
    }
    
    // Reconstruct all vectors
    auto all_vectors = reconstruct_all();
    if (all_vectors.size() != labels_.size()) {
        std::cerr << "Warning: Vector count mismatch during reconstruction" << std::endl;
        // Continue with best effort
    }
    
    // Build new vectors and labels arrays with only kept entries
    std::vector<std::vector<float>> new_vectors;
    std::vector<std::string> new_labels;
    
    for (size_t idx : keep_indices) {
        if (idx < labels_.size()) {
            new_labels.push_back(labels_[idx]);
            if (idx < all_vectors.size()) {
                new_vectors.push_back(all_vectors[idx]);
            }
        }
    }
    
    // Rebuild the index with remaining vectors
    if (!new_vectors.empty() && new_vectors.size() == new_labels.size()) {
        // Convert to format expected by add method
        std::vector<std::vector<float>> vectors_batch;
        for (const auto& vec : new_vectors) {
            vectors_batch.push_back(vec);
        }
        
        // Clear current state
        labels_.clear();
        if (create_index()) {
            // Add all kept vectors
            if (add(new_labels, vectors_batch)) {
                return removed_count;
            }
        }
    }
    
    std::cerr << "Failed to rebuild index after removal" << std::endl;
    return 0;
#else
    return 0;
#endif
