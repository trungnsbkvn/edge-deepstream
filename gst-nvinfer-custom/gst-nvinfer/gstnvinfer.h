...
struct _GstNvInfer
{
  ...
  // Alignment perf counters (windowed)
  unsigned long long align_window_time_us = 0;   // total time in current window (microseconds)
  unsigned long long align_window_count = 0;     // crops aligned in current window
  unsigned long long align_last_log_ns = 0;      // last log time (nanoseconds since steady epoch)
};
...
typedef struct LandmarkInfo
{
  ...
  int track_id;
  int frame_id;
  // absolute rect in input frame
  int rect_left;
  int rect_top;
  int rect_width;
  int rect_height;
  // conversion params used in get_converted_buffer
  double ratio_x;
  double ratio_y;
