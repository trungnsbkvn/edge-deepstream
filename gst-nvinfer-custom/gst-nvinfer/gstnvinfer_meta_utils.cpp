...
/* copy function for string meta (aligned image path) */
static gpointer copy_string_user_meta(gpointer data, gpointer user_data)
{
  NvDsUserMeta *user_meta = (NvDsUserMeta *)data;
  const char *src = (const char*)user_meta->user_meta_data;
  if (!src) return NULL;
  size_t len = strlen(src) + 1;
  char *dst = (char*)g_malloc(len);
  memcpy(dst, src, len);
  return (gpointer)dst;
}

/* release function for string meta */
static void release_string_user_meta(gpointer data, gpointer user_data)
{
  NvDsUserMeta *user_meta = (NvDsUserMeta *) data;
  if (user_meta->user_meta_data) {
    g_free(user_meta->user_meta_data);
...
void *set_metadata_ptr(const std::vector<std::string>& tokens, GstNvInfer * nvinfer, NvDsObjectMeta *parent_obj_meta, GstNvInferFrame & frame){
  ...
  for (unsigned int i = 0; i < tokens.size() - 1; ) {
    if (i + 1 < tokens.size() && i + 2 < tokens.size()) {
      ...
      // 处理 tokens 容器大小不足的情况
      std::cerr << "Not enough tokens in the vector." << std::endl;
      break; // 或者其他适当的错误处理
    }
  }
...
{
  if (!obj_meta || !path_str || !*path_str) return;
  ...
  user_meta->base_meta.meta_type = (NvDsMetaType)NVDS_USER_OBJECT_META_ALIGNED_IMAGE_PATH;
  user_meta->base_meta.copy_func = (NvDsMetaCopyFunc)copy_string_user_meta;
  user_meta->base_meta.release_func = (NvDsMetaReleaseFunc)release_string_user_meta;
