...
static bool align_preprocess(GstNvInfer *nvinfer, NvBufSurface * frame_surface,
                             NvBufSurface * surface, GstNvInferMemory* alloc_mem,
                             std::vector<LandmarkInfo>& landmarkInfos);
...
gst_nvinfer_init (GstNvInfer * nvinfer)
{
  ...
  // Init perf counters
  nvinfer->align_window_time_us = 0;
  nvinfer->align_window_count = 0;
  nvinfer->align_last_log_ns = 0;
...
static bool
align_preprocess(GstNvInfer *nvinfer, NvBufSurface * frame_surface, NvBufSurface * surface, GstNvInferMemory* alloc_mem, std::vector<LandmarkInfo>& landmarkInfos){
  // Ensure we operate on the correct CUDA device to avoid invalid pointer copies
  cudaError_t _cudarc = cudaSetDevice(nvinfer->gpu_id);
  if (_cudarc != cudaSuccess) {
    GST_ERROR_OBJECT(nvinfer, "cudaSetDevice(%d) failed: %s", (int)nvinfer->gpu_id, cudaGetErrorString(_cudarc));
    return false;
  }
  if (frame_surface->numFilled != surface->numFilled) {
    GST_ERROR_OBJECT(nvinfer, "Expected same number of frames in batch, but got frame: %d, surface: %d", frame_surface->numFilled, surface->numFilled);
    return false;
  }
  // timer vars (use steady_clock for perf)
  ...
  NvBufSurfaceColorFormat frame_color_format = surface->surfaceList[0].colorFormat;
  int channels = 0;
  switch (frame_color_format) {
    case NVBUF_COLOR_FORMAT_RGB:
    ...
    case NVBUF_COLOR_FORMAT_BGRA:
      channels = 4; break;
    default:
      GST_WARNING_OBJECT(nvinfer, "Unsupported color format %d for alignment; expected RGB/BGR/RGBA/BGRA", frame_color_format);
      return false;
  }
  size_t row_bytes = (size_t)frame_width * (size_t)channels;

  size_t cropped_datasize  = surface->surfaceList[0].dataSize;
  // Determine pitched size (in bytes) for allocation; use plane pitch to avoid 2D copies
  int pitch_bytes0 = surface->surfaceList[0].planeParams.pitch[0];
  if (pitch_bytes0 <= 0) {
    GST_ERROR_OBJECT(nvinfer, "Invalid pitch (0) on surface for alignment");
    return false;
  }
  size_t host_pitch_bytes = (size_t)pitch_bytes0;
  size_t host_buf_size = host_pitch_bytes * (size_t)frame_height;
  bool do_vis = false;
  if (strlen(nvinfer->alignment_pic_path)) {
    // Reuse pitched sizing for debug buffers too
    cudaError_t verr1 = cudaMallocHost(&host_aligned_ptr, host_buf_size);
    cudaError_t verr2 = cudaMallocHost(&host_origin_ptr, host_buf_size);
    if (verr1 != cudaSuccess || verr2 != cudaSuccess) {
      GST_ERROR_OBJECT(nvinfer, "Failed to allocate pinned host buffers for alignment");
      if (host_aligned_ptr) cudaFreeHost(host_aligned_ptr), host_aligned_ptr = nullptr;
      if (host_origin_ptr) cudaFreeHost(host_origin_ptr), host_origin_ptr = nullptr;
      do_vis = false;
    } else {
      do_vis = true;
    }
  ...
  for (uint frameIndex = 0; frameIndex < surface->numFilled; frameIndex++) {
    ...
    // Basic sanity checks on pointers and pitches
    void* dev_ptr = surface->surfaceList[frameIndex].dataPtr;
    // If the surface is SURFACE_ARRAY, prefer using allocator-provided CUDA pointer
    if (surface->memType == NVBUF_MEM_SURFACE_ARRAY && alloc_mem) {
      if (frameIndex < alloc_mem->frame_memory_ptrs.size() && alloc_mem->frame_memory_ptrs[frameIndex]) {
        dev_ptr = alloc_mem->frame_memory_ptrs[frameIndex];
      }
    }
    int src_pitch = surface->surfaceList[frameIndex].planeParams.pitch[0];
    int src_height = (int)surface->surfaceList[frameIndex].height;
    int src_width = (int)surface->surfaceList[frameIndex].width;
    if (!dev_ptr || src_pitch <= 0 || src_height <= 0 || src_width <= 0) {
      GST_WARNING_OBJECT(nvinfer, "Invalid surface plane for alignment (ptr=%p, pitch=%d, w=%d, h=%d)",
                         dev_ptr, src_pitch, src_width, src_height);
      continue;
    }
    // Validate CUDA pointer attributes to avoid illegal address issues on non-device memory
    cudaPointerAttributes attrs{};
    cudaError_t aerr = cudaPointerGetAttributes(&attrs, dev_ptr);
#if CUDART_VERSION >= 10000
    bool is_dev_ptr = (aerr == cudaSuccess) && (attrs.type == cudaMemoryTypeDevice || attrs.type == cudaMemoryTypeManaged);
    int ptr_dev = (aerr == cudaSuccess) ? attrs.device : -1;
#else
    bool is_dev_ptr = (aerr == cudaSuccess) && (attrs.memoryType == cudaMemoryTypeDevice);
    int ptr_dev = (aerr == cudaSuccess) ? attrs.device : -1;
#endif
    // Trust NvBufSurface mem types as GPU-capable: DEFAULT, CUDA_DEVICE, SURFACE_ARRAY.
    // Only skip if memType is SYSTEM or pointer is null. Attribute checks can be unreliable with EGL.
    bool mem_gpu_capable = (surface->memType == NVBUF_MEM_DEFAULT ||
                            surface->memType == NVBUF_MEM_CUDA_DEVICE ||
                            surface->memType == NVBUF_MEM_SURFACE_ARRAY);
    if (!mem_gpu_capable) {
      GST_WARNING_OBJECT(nvinfer, "Surface memType %d not GPU-capable; skipping warp", (int)surface->memType);
      continue;
    }
    if (!is_dev_ptr || (ptr_dev != -1 && ptr_dev != (int)nvinfer->gpu_id)) {
      GST_DEBUG_OBJECT(nvinfer, "Relaxed ptr attr check (ok=%d, type_ok=%d, dev=%d, expect=%d) for memType=%d; proceeding",
                       (int)(aerr==cudaSuccess), (int)is_dev_ptr, ptr_dev, (int)nvinfer->gpu_id, (int)surface->memType);
    }
  // GPU path: no CPU mapping needed
  size_t copy_bytes = (size_t)src_pitch * (size_t)src_height;
  
    int row = sizeof(lmks) / sizeof(lmks[0]);
    cv::Mat dst(row ,2, CV_32FC1, lmks);
    memcpy(dst.data, lmks, 2 * row * sizeof(float));
    // Map absolute (global) landmarks into cropped surface coordinates if provided
    if (lmkinfo.rect_width > 0 && lmkinfo.rect_height > 0) {
      for (int i = 0; i < 5; ++i) {
        float x_abs = dst.at<float>(i,0);
        float y_abs = dst.at<float>(i,1);
        // Convert from absolute frame coords to pre-scale ROI coords
        float x_roi = (x_abs - lmkinfo.rect_left);
        float y_roi = (y_abs - lmkinfo.rect_top);
        // Apply scale into destination surface with padding offsets
        float x_dst = (float)(x_roi * lmkinfo.ratio_x + lmkinfo.offset_left);
        float y_dst = (float)(y_roi * lmkinfo.ratio_y + lmkinfo.offset_top);
        dst.at<float>(i,0) = x_dst;
        dst.at<float>(i,1) = y_dst;
      }
    }
    // Compute similarity (src->dst) for OpenCV warpAffine
    cv::Mat M = nvinfer->aligner.Align(dst, nvinfer->alignment_type);
    if (M.empty() || M.rows != 3 || M.cols != 3) {
      GST_WARNING_OBJECT(nvinfer, "Invalid alignment matrix shape %dx%d, skip alignment", M.rows, M.cols);
      continue;
    }
    // Guard against NaN/Inf in M
    bool bad=false; for(int r=0;r<2;r++){ for(int c=0;c<3;c++){ float v=M.at<float>(r,c); if(!std::isfinite(v)) { bad=true; break; } } if(bad) break; }
    if (bad) {
      GST_WARNING_OBJECT(nvinfer, "Alignment matrix contains NaN/Inf; skipping warp");
      continue;
    }
    ...
    if (cropped_frame_width <= 0 || cropped_frame_height <= 0) {
      GST_WARNING_OBJECT(nvinfer, "Invalid crop size, skip alignment: %dx%d",
        cropped_frame_width, cropped_frame_height);
      continue;
    }

    if (debug_counter < 5) {
      GST_INFO_OBJECT(nvinfer, "Align lmks: (%.1f,%.1f) (%.1f,%.1f) (%.1f,%.1f) (%.1f,%.1f) (%.1f,%.1f)",
        lmks[0][0], lmks[0][1], lmks[1][0], lmks[1][1], lmks[2][0], lmks[2][1], lmks[3][0], lmks[3][1], lmks[4][0], lmks[4][1]);
      GST_INFO_OBJECT(nvinfer, "Affine M: [%.6f %.6f %.6f; %.6f %.6f %.6f] dsize=%dx%d src=%dx%d",
        M.at<float>(0,0), M.at<float>(0,1), M.at<float>(0,2),
        M.at<float>(1,0), M.at<float>(1,1), M.at<float>(1,2),
        cropped_frame_width, cropped_frame_height, frame_width, frame_height);
      debug_counter++;
    }
    // Optionally capture origin for visualization before GPU warp
  if (do_vis) {
      cudaMemcpy(host_origin_ptr, dev_ptr, copy_bytes, cudaMemcpyDeviceToHost);
    }

    // GPU warp: compute inverse 2x3 for dst->src mapping and run CUDA kernel
    auto crop_start = clock::now();
    cv::Mat M2x3 = M(cv::Rect(0,0,3,2));
    cv::Mat M2x3f; M2x3.convertTo(M2x3f, CV_32F);
    cv::Mat M3x3 = cv::Mat::eye(3,3,CV_32F);
    M2x3f.copyTo(M3x3(cv::Rect(0,0,3,2)));
    cv::Mat M3x3_inv = M3x3.inv();
    float invM2x3[6] = {
      M3x3_inv.at<float>(0,0), M3x3_inv.at<float>(0,1), M3x3_inv.at<float>(0,2),
      M3x3_inv.at<float>(1,0), M3x3_inv.at<float>(1,1), M3x3_inv.at<float>(1,2)
    };
    uint8_t* d_dst_tmp = nullptr;
    cudaError_t cerr;
    cerr = cudaMalloc(&d_dst_tmp, copy_bytes);
    if (cerr != cudaSuccess || d_dst_tmp == nullptr) {
      GST_ERROR_OBJECT(nvinfer, "cudaMalloc(%zu) failed for d_dst_tmp: %s", copy_bytes, cudaGetErrorString(cerr));
      // Skip this crop but continue pipeline
      continue;
    }
  alignnamespace::Aligner::WarpAffineU8CUDA(
      (const uint8_t*)dev_ptr, src_width, src_height, src_pitch,
      d_dst_tmp, cropped_frame_width, cropped_frame_height, src_pitch,
      invM2x3, channels, 0, nvinfer->convertStream);
    // Check kernel launch status
    cerr = cudaGetLastError();
    if (cerr != cudaSuccess) {
      GST_ERROR_OBJECT(nvinfer, "WarpAffineU8CUDA launch failed: %s", cudaGetErrorString(cerr));
      cudaFree(d_dst_tmp);
      continue;
    }
    cerr = cudaMemcpyAsync(dev_ptr, d_dst_tmp, copy_bytes, cudaMemcpyDeviceToDevice, nvinfer->convertStream);
    if (cerr != cudaSuccess) {
      GST_ERROR_OBJECT(nvinfer, "cudaMemcpyAsync D2D failed: %s", cudaGetErrorString(cerr));
      cudaFree(d_dst_tmp);
      continue;
    }
    cerr = cudaStreamSynchronize(nvinfer->convertStream);
    if (cerr != cudaSuccess) {
      GST_ERROR_OBJECT(nvinfer, "convertStream sync failed: %s", cudaGetErrorString(cerr));
      cudaFree(d_dst_tmp);
      continue;
    }
    cudaFree(d_dst_tmp);

    // perf: per-crop time and windowed FPS (~2s window)
    auto crop_end = clock::now();
    auto us = std::chrono::duration_cast<std::chrono::microseconds>(crop_end - crop_start).count();
    nvinfer->align_window_time_us += (unsigned long long)us;
    nvinfer->align_window_count += 1;
    if (debug_counter < 5) {
      GST_INFO_OBJECT(nvinfer, "Align crop took %lld us (channels=%d, %dx%d)", (long long)us, channels, cropped_frame_width, cropped_frame_height);
      // ensure we don't spam logs beyond the initial crops
      debug_counter++;
    }
    // Log FPS every ~2 seconds
    auto now_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(crop_end.time_since_epoch()).count();
    if (nvinfer->align_last_log_ns == 0) nvinfer->align_last_log_ns = now_ns;
    const long long interval_ns = 2LL * 1000 * 1000 * 1000; // 2 seconds
    if ((long long)(now_ns - nvinfer->align_last_log_ns) >= interval_ns && nvinfer->align_window_count > 0) {
      double avg_us = (double)nvinfer->align_window_time_us / (double)nvinfer->align_window_count;
      double fps = 1e6 / std::max(1.0, avg_us);
      GST_INFO_OBJECT(nvinfer, "Alignment FPS ~ %.2f (avg %.0f us over %llu crops)", fps, avg_us, (unsigned long long)nvinfer->align_window_count);
      nvinfer->align_window_time_us = 0;
      nvinfer->align_window_count = 0;
  ...
  if (do_vis) {
      ...
      // Views into pitched host buffers
      cv::Mat view_aligned, view_origin;
      if (channels == 3) {
        view_aligned = cv::Mat(cropped_frame_height, cropped_frame_width, CV_8UC3, host_aligned_ptr, (size_t)src_pitch);
        view_origin  = cv::Mat(cropped_frame_height, cropped_frame_width, CV_8UC3, host_origin_ptr,  (size_t)src_pitch);
      } else {
        view_aligned = cv::Mat(cropped_frame_height, cropped_frame_width, CV_8UC4, host_aligned_ptr, (size_t)src_pitch);
        view_origin  = cv::Mat(cropped_frame_height, cropped_frame_width, CV_8UC4, host_origin_ptr,  (size_t)src_pitch);
      }

  // Convert to BGR for correct colors when saving with OpenCV
      cv::Mat vis_aligned, vis_origin;
      switch (frame_color_format) {
        case NVBUF_COLOR_FORMAT_RGB:
          cv::cvtColor(view_aligned, vis_aligned, cv::COLOR_RGB2BGR);
          cv::cvtColor(view_origin,  vis_origin,  cv::COLOR_RGB2BGR);
          break;
        case NVBUF_COLOR_FORMAT_BGR:
          vis_aligned = view_aligned.clone();
          vis_origin  = view_origin.clone();
          break;
        case NVBUF_COLOR_FORMAT_RGBA:
          cv::cvtColor(view_aligned, vis_aligned, cv::COLOR_RGBA2BGR);
          cv::cvtColor(view_origin,  vis_origin,  cv::COLOR_RGBA2BGR);
          break;
        case NVBUF_COLOR_FORMAT_BGRA:
          cv::cvtColor(view_aligned, vis_aligned, cv::COLOR_BGRA2BGR);
          cv::cvtColor(view_origin,  vis_origin,  cv::COLOR_BGRA2BGR);
          break;
        default:
          // Fallback: best-effort clone
          vis_aligned = view_aligned.clone();
          vis_origin  = view_origin.clone();
          break;
      }

      // Draw landmarks on origin (in crop space)
      for (int i = 0; i < 5; ++i) {
        cv::Point2f p(dst.at<float>(i,0), dst.at<float>(i,1));
        cv::circle(vis_origin, p, 2, cv::Scalar(0, 255, 0), -1, cv::LINE_AA);
      }
      // Project landmarks with M to aligned space and draw for quick sanity
      cv::Mat M2x3_draw = M(cv::Rect(0,0,3,2));
      for (int i = 0; i < 5; ++i) {
        float x = dst.at<float>(i,0), y = dst.at<float>(i,1);
        float xa = M2x3_draw.at<float>(0,0)*x + M2x3_draw.at<float>(0,1)*y + M2x3_draw.at<float>(0,2);
        float ya = M2x3_draw.at<float>(1,0)*x + M2x3_draw.at<float>(1,1)*y + M2x3_draw.at<float>(1,2);
        cv::circle(vis_aligned, cv::Point2f(xa, ya), 2, cv::Scalar(0, 0, 255), -1, cv::LINE_AA);
      }

      // Filenames
      char image_aligned_name[strlen(nvinfer->alignment_pic_path)+128];
      char image_origin_name[strlen(nvinfer->alignment_pic_path)+128];
      snprintf(image_aligned_name, sizeof(image_aligned_name), "%s/frame-%d_object-%d-aligned.png",
               nvinfer->alignment_pic_path, lmkinfo.frame_id, lmkinfo.track_id);
      snprintf(image_origin_name, sizeof(image_origin_name), "%s/frame-%d_object-%d-origin.png",
               nvinfer->alignment_pic_path, lmkinfo.frame_id, lmkinfo.track_id);

  // Save
  cv::imwrite(image_aligned_name, vis_aligned);
  cv::imwrite(image_origin_name,  vis_origin);

  // Attach the aligned image path as user meta to the corresponding object
  // We attempt to use the nth frame in current batch for mapping (frameIndex)
  // Note: align_preprocess is called from convert_batch_and_push_to_input_thread_alignment
  // with the same ordering as batch->frames.
  // To bridge context, we can add a lightweight ring or attach to the latest object in landmarkInfos prior to erase.
  // Here we cannot access object_meta directly; meta will be attached right after conversion step below using landmark info.
    }
  }
  
  // no CPU warp buffers to free
  if (strlen(nvinfer->alignment_pic_path)) {
    cudaFreeHost(host_aligned_ptr);
    cudaFreeHost(host_origin_ptr);
  }
  // all crops done; optional end-to-end time for batch
  auto t1 = clock::now();
  auto batch_us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
  GST_DEBUG_OBJECT(nvinfer, "Alignment batch completed in %lld us for %u crops", (long long)batch_us, surface->numFilled);
  return true;
}
...
convert_batch_and_push_to_input_thread_alignment (GstNvInfer *nvinfer,
    GstNvInferBatch *batch, GstNvInferMemory *mem, std::vector<LandmarkInfo>& landmarkInfos)
{
  NvBufSurfTransform_Error err = NvBufSurfTransformError_Success;
  std::string nvtx_str;
  ...
  /* Ensure the async transform is complete before alignment to avoid races */
  if (batch->sync_obj) {
    NvBufSurfTransformSyncObjWait (batch->sync_obj, -1);
    NvBufSurfTransformSyncObjDestroy (&(batch->sync_obj));
    batch->sync_obj = nullptr;
  }

  bool ret = align_preprocess(nvinfer, &nvinfer->tmp_surf, mem->surf, mem, landmarkInfos);

  // Attach aligned image path meta per object if saving path is enabled
  if (ret && nvinfer->alignment_pic_path && strlen(nvinfer->alignment_pic_path)) {
    for (size_t i = 0; i < batch->frames.size(); ++i) {
      GstNvInferFrame &f = batch->frames[i];
      if (!f.obj_meta) continue;
      char image_aligned_name[strlen(nvinfer->alignment_pic_path)+128];
      snprintf(image_aligned_name, sizeof(image_aligned_name), "%s/frame-%ld_object-%d-aligned.png",
               nvinfer->alignment_pic_path, (long)f.frame_num, (int)f.obj_meta->object_id);
      attach_aligned_image_path_meta(nvinfer, f.obj_meta, image_aligned_name);
    }
  }
    ...
    NvBufSurface * in_surf)
{
  ...
  for (NvDsMetaList * l_frame = batch_meta->frame_meta_list; l_frame != NULL;
    ...
    for (NvDsMetaList * l_obj = frame_meta->obj_meta_list; l_obj != NULL;
      ...
      // Back-fill latest landmark info with rect and transform params
      if (nvinfer->alignment_type && !landmarkInfos.empty()) {
        LandmarkInfo &info = landmarkInfos.back();
        info.rect_left = (int)object_meta->rect_params.left;
        info.rect_top = (int)object_meta->rect_params.top;
        info.rect_width = (int)object_meta->rect_params.width;
        info.rect_height = (int)object_meta->rect_params.height;
        info.ratio_x = scale_ratio_x;
        info.ratio_y = scale_ratio_y;
        info.offset_left = offset_left;
        info.offset_top = offset_top;
      }
      ...
      if (batch->frames.size () == nvinfer->max_batch_size && nvinfer->alignment_type) {
        if (!convert_batch_and_push_to_input_thread_alignment (nvinfer, batch.get(), memory, landmarkInfos)) {
          return GST_FLOW_ERROR;
        }
        batch.release ();
        landmarkInfos.clear();
  ...
  if (batch) {
    ...
    } else if (!convert_batch_and_push_to_input_thread (nvinfer, batch.get(), memory)) {
      return GST_FLOW_ERROR;
    }
    conv_gst_buf = nullptr;
    batch.release ();
...
nvinfer_plugin_init (GstPlugin * plugin)
{
  ...
  GST_INFO("[CUSTOM GST-NVINFER] plugin init: edge-deepstream custom build loaded");
